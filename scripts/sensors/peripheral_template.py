...
class PeripheralTemplate:
    ...
    def poll(self):
        """
        Called by main.py polling loop.
        
        Read sensor and return OSC message to send, or None.
        
        Returns:
            tuple: (osc_path, osc_args) to send to Pure Data, or None
        
        Examples:
            return ("/mydevice/data", [123, 456])
            return ("/mydevice/value", [self.read_data()['value']])
            return None  # Don't send anything
        """
        # Example: Always send current value
        # data = self.read_data()
        # return (f"/{self.name}/data", [data['value']])
        
        # Example: Only send if value changed
        # current = self.read_data()['value']
        # if current != self.last_value:
        #     self.last_value = current
        #     return (f"/{self.name}/changed", [current])
        
        # Example: Don't auto-send (only respond to OSC requests)
        return None
...
#     
#   def poll(self):
#         # Only send data if temperature changed by more than 0.5 degrees      
#         data = self.read_data()
#         if abs(data['celsius'] - getattr(self, 'last_celsius', None)) > 0.5:
#             self.last_celsius = data['celsius']   
